...
func resourceSharedFlowDeploymentRead(d *schema.ResourceData, meta interface{}) (e error) {
	...
	if found {
		log.Printf("[DEBUG] resourceSharedFlowDeploymentRead - deployment found. Revision is: %#v", latestRevision)
...
func resourceSharedFlowDeploymentCreate(d *schema.ResourceData, meta interface{}) error {
	...
	shared_flow_name := d.Get("shared_flow_name").(string)
	...
	if d.Get("revision").(string) == "latest" {
		// deploy latest
		rev, err := getLatestRevision(client, shared_flow_name)
		if err != nil {
			return fmt.Errorf("[ERROR] resourceSharedFlowDeploymentUpdate error getting latest revision: %v", err)
		}
		_, _, err = client.SharedFlows.Deploy(shared_flow_name, env, apigee.Revision(rev), delay, override)
		if err != nil {
			return fmt.Errorf("[ERROR] resourceSharedFlowDeploymentUpdate error deploying: %v", err)
		}
		log.Printf("[DEBUG] resourceSharedFlowDeploymentUpdate Deployed revision %d of %s", rev, shared_flow_name)
		return resourceSharedFlowDeploymentRead(d, meta)
	}

	sharedFlowDep, _, err := client.SharedFlows.Deploy(shared_flow_name, env, rev, delay, override)
...
func resourceSharedFlowDeploymentUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("revision").(string) == "latest" {
		...
		_, _, err = client.SharedFlows.ReDeploy(shared_flow_name, env, apigee.Revision(rev), delay, override)
	...
	rev_int, _ := strconv.Atoi(d.Get("revision").(string))
	rev := apigee.Revision(rev_int)
	_, _, err := client.SharedFlows.ReDeploy(shared_flow_name, env, rev, delay, override)
...
func resourceSharedFlowDeploymentDelete(d *schema.ResourceData, meta interface{}) error {
	...
	_, _, err := client.SharedFlows.Undeploy(shared_flow_name, env, rev)
